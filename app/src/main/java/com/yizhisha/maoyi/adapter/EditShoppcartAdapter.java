...
import com.yizhisha.maoyi.widget.BaseFlowLayout;
...
public class EditShoppcartAdapter extends BaseQuickAdapter<GoodsAttrsBean.AttributesBean,BaseViewHolder> {
    ...
    public EditShoppcartAdapter(@Nullable List<GoodsAttrsBean.AttributesBean> list,List<GoodsAttrsBean.StockGoodsBean> stockGoodsList) {
        super(R.layout.dialog_shoppcart_edit_item,list);
    ...
    @Override
    protected void convert(BaseViewHolder helper,GoodsAttrsBean.AttributesBean item) {
        TextView tv_ItemName = helper.getView(R.id.tv_ItemName);
        BaseFlowLayout mFlowLayout = (BaseFlowLayout) helper.getView(R.id.news_top_title_base_flowlayout);
        ...
        for (int i = 0; i < childrenSize; i++) {
            TextView textView = (TextView) LayoutInflater.from(mContext).inflate(R.layout.news_top_title_search_, mFlowLayout, false);
            textView.setText(childrens.get(i));
            textView.setTextColor(ContextCompat.getColor(mContext, R.color.common_h1));
            textViews[i] = textView;
            mFlowLayout.addView(textViews[i]);
        }
        childrenViews[helper.getAdapterPosition()] = textViews;
        initOptions();
        canClickOptions();
        getSelected();
    }
    ...
    private void initOptions() {
        for (int y = 0; y < childrenViews.length; y++) {
            for (int z = 0; z < childrenViews[y].length; z++) {//循环所有属性
                ...
                textView.setTextColor(ContextCompat.getColor(mContext, R.color.common_h3));//变灰
                textView.setBackgroundResource(R.drawable.news_top_title_search_bg);
            }
        }
    }
            ...
            for (int j = 0; j < childrenViews[i].length; j++) {//拿到每行属性Item
                ...
                for (int m = 0; m < selectedValue.length; m++) {
                    if (selectedValue[m].equals(value)) {
                        textView.setTextColor(RescourseUtil.getColor(R.color.white));
                        textView.setBackgroundResource(R.drawable.news_top_title_search_label_bg);
