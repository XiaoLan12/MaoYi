...
public class ImageSlideshow extends FrameLayout {
    ...
    private List<BannerResult> imageBanner;
    ...
    public void setImageTitleBeanList(List<BannerResult> imageBeanList) {
        this.imageBanner = imageBeanList;
    }

    // 设置完后最终提交
    public void commit() {
        if (imageBanner != null) {
            count = imageBanner.size();
            // 设置ViewPager
            setViewPager(imageBanner);
            // 设置指示器
            setIndicator();
            // 开始播放
            starPlay();
     ...
     */
    private void setViewPager(List<BannerResult> imageTitleBeanList) {
        // 设置View列表
     ...
     * @param imageTitleBeanList
     */
    private void setViewList(List<BannerResult> imageTitleBeanList) {
        ...
        for (int i = 0; i < count + 2; i++) {
            ...
            if (i == 0) {// 将最前面一页设置成本来最后的那页
                Glide.with(context).
                        load(imageTitleBeanList.get(count - 1).getUrl()).into(ivImage);
            } else if (i == count + 1) {// 将最后面一页设置成本来最前的那页
                Glide.with(context).
                        load(imageTitleBeanList.get(0).getUrl()).into(ivImage);
            } else {
                Glide.with(context).
                        load(imageTitleBeanList.get(i - 1).getUrl()).into(ivImage);
            }
            // 将设置好的View添加到View列表中
            viewList.add(view);
        }
